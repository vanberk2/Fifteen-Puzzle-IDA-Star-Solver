...
public class IDAStarSearchMisplacedT {
    ...
    public void do_MisplacedTSearch (Node root, Timer timer) {
        int bound = root.getMTH();
        Stack<Node> explored = new Stack<>();
        explored.push(root);

        while (true) {
            b = r_aStarSearchMisplacedT(timer, 0, bound, explored);
            if (b == -1)
                break;
            bound = b;
        }
    ...
    public int r_aStarSearchMisplacedT (Timer timer, int g, int bound, Stack<Node> explored) {
        ...
        for (int i = 0; i < curr.children.size(); i++) {
            ...
            if (!hasChild(currChild, explored)) {
                ...
                nodesExpanded++;
                b = r_aStarSearchMisplacedT(timer,  g + 1, bound, explored);
                if (b == -1)
                    return b;
                if (b < min)
                    min = b;
